...
class SettingsController extends ApiBaseController
{
    ...
    public function paymentKeySave(){
        $request = request();
        $warehouse = warehouse();

        $setting = Settings::where('other_data',$warehouse->id)->where("name_key",$request->paymentKey)->first();



        if($setting == null){
            $settings = new Settings();
            $settings->name_key = $request->paymentKey;
            $settings->is_global = 0;
            $settings->setting_type = 'payment_key';
            $settings->name = "warehouse payment key";
            $settings->other_data= $warehouse->id;
            $settings->credentials = $request->credentials;
            $settings->save();
            return ApiResponse::make('Created Success', [$settings]);
        }else{
             $setting->credentials = $request->credentials;
             $setting->save();
             return ApiResponse::make('Updated Success', [$setting]);
        }
    ...
    public function getPaymentKey(){
        ...
        return ApiResponse::make('Success', ['data'=>$setting]);
