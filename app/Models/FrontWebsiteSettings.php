...
class FrontWebsiteSettings extends BaseModel
{
    ...
    public function getFeaturedProductsDetailsAttribute()
    {
        ...
        // Added by 'Parthiban' for getting the product's Sales_price with tax calculated.
        foreach ($products as $key => $product) {
            if ($product->details->sales_tax_type == "exclusive") {
                if (!$product->details->tax) {
                    $product->details->tax = (object)['rate' => 0];
                }
                $product->details->sales_price = ($product->details->sales_price + ($product->details->sales_price * ($product->details->tax->rate / 100)));
            }
        }
