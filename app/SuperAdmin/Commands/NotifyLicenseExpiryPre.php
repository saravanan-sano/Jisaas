...
use App\SuperAdmin\Notifications\LicenseExpirePre;
use App\Models\User;
use Carbon\Carbon;
        ...
        if (!is_null($trialPlan->notify_before)) {
            ...
            foreach ($companiesNotify as $cmp) {
                ...
                //    $companyUser->notify(new LicenseExpirePre(($companyUser)));
                   ...
                   $licenceExpireOn = Carbon::parse($cmp->licence_expire_on)->format('Y-m-d');


                 $notficationData = [
                    'expiry_date' => $licenceExpireOn,
                ];
                Notification::route('mail', $companyUser->email)->notify(new LicenseExpirePre($notficationData));
                }
            }
        }
    }
