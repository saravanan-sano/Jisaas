...
use App\SuperAdmin\Http\Requests\Api\PaymentSettings\UpiIndexRequest;
use App\SuperAdmin\Http\Requests\Api\PaymentSettings\AuthorizeUpdateRequest;
use App\SuperAdmin\Http\Requests\Api\PaymentSettings\UpiUpdateRequest;
...
class PaymentSettingsController extends ApiBaseController
{
    ...
    public function getUpi(UpiIndexRequest $request)
    {
        $settings = GlobalSettings::where('setting_type', 'payment_settings')
            ->where('name_key', 'upi')
            ->first();
        ...
        $settingData = [
            'upi_gateway_key' => $settings->credentials['upi_gateway_key'],
            'upi_status' => $settings->credentials['upi_status'],
        ];
        ...
        return ApiResponse::make(
            ...
            [
                'data' => $settingData,
                'webhook_url' => route('webhook.save-upi-invoices')
            ]
        );
    }

    public function updateUpi(UpiUpdateRequest $request)
    {
        $settingData = [
            'upi_gateway_key' => $request->upi_gateway_key,
            'upi_status' => $request->upi_status,
        ];
        ...
        GlobalSettings::where('setting_type', 'payment_settings')
            ...
            ->update([
                'credentials' => $settingData,
                'status' => $request->upi_status == 'active' ? 1 : 0
            ]);
