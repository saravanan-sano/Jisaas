...
class RazorpayWebhookController extends Controller
{
    ...
    protected function paymentAuthorized(array $requestData)
    {
        ...
        $plan = SubscriptionPlan::find(Hashids::decode($packageId));

        $idCompany = Hashids::decode($companyID);
        $company = Company::findOrFail($idCompany[0]);
        ...
        try {
            if ($company) {
                ...
                $stripeInvoice->company_id      = $company->id;     //company ID hashed
                // $stripeInvoice->currency_id     = $currencyID;
                // $stripeInvoice->order_id        = $orderID;         //order_id not in the table
                ...
                $subscriptionPlanId = Hashids::decode($packageId);
                $stripeInvoice->subscription_plan_id      = $subscriptionPlanId[0];
