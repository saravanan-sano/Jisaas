...
class CompanyScope implements Scope
{
    public function apply(Builder $builder, Model $model)
    {
        ...
        if (app_type() == 'multiple' && auth('api')->check() && Schema::hasColumn($model->getTable(), 'company_id')) {
            $company = company();
        ...
        } else if (app_type() == 'single' && auth('api')->check() && Schema::hasColumn($model->getTable(), 'company_id')) {
            $company = company();
