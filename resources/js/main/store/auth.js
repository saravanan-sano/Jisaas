...
const HELP_VIDEO = "helpvideos";
const EXPORT_EXCEL = "updatAllExportData";
...
export default {
        ...
        return {
           ...
           // appSetting: getJSONFromLocalStorage(APP_SETTINGS_KEY),
            ...
            country_code:
                ...
                window.config.country_code,
                allExportData:
                window.localStorage.getItem(EXPORT_EXCEL) || null
    ...
    mutations: {
        ...
        updatePageTitle(state, pageTitle) {
            ...
            if (state.appSetting !== null && state.appSetting !== undefined) {
               // document.title = `${pageTitle} - ${state.appSetting.name}`;
                document.title = `${state.appSetting.name}`;

            } else {
                document.title = `${pageTitle} - jierp`;
            }
        },
        ...
        updateCssSettings(state, cssSettings) {
            ...
            window.localStorage.setItem(
                ...
                JSON.stringify({
                    ...
                    headerMenuMode:
                        cssSettings == "sidebar" ? "vertical" : "horizontal",
                })
            );
        },
        ...
        updateCountryCode(state, modules) {
            window.localStorage.setItem(
                COUNTRY_CODE,
                JSON.stringify(modules)
            );
        ...
        updateHelpVideos(state, modules) {
            window.localStorage.setItem(
                HELP_VIDEO,
                JSON.stringify(modules)
            );
        },
        updatAllExportData(state, modules) {
            state.allExportData = modules;
            window.localStorage.setItem(
                EXPORT_EXCEL,
                JSON.stringify(modules)
            );
