                    ...
                    >
                        <a-button :loading="loading" type="primary">
                            <a-typography-link
                                ...
                                download
                            >
                                Click here to download sample xlsx file
                            </a-typography-link>
                        </a-button>
        ...
        <a-form layout="vertical">
            ...
            <a-row :gutter="16">
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                    <a-form-item
                        :label="$t('stock.attachments')"
                        name="attachments"
                        :help="
                            rules.attachments ? rules.attachments.message : null
                        "
                        :validateStatus="rules.attachments ? 'error' : null"
                    >
                        <UploadFile
                            ...
                            @onFileUploaded="
                                (file) => {
                                    formData.attachments = file.file;
                                    formData.attachments_url = file.file_url;
                                }
                            "
                        />
                    </a-form-item>
                ...
                <a-col
                    ...
                    :md="12"
                    :lg="12"
                    v-if="appSetting.is_purchases_attachment === 0"
                >
                    <a-alert
                        :description="$t('messages.is_purchases_attachment')"
                        type="warning"
                        showIcon
                    />
                </a-col>
                            ...
                            <template v-if="column.dataIndex === 'sales_price'">
                                ...
                                >
                                    <a-input-number
                                        ...
                                        style="width: 90px"
                                        @change="
                                            () => handleSalesPriceChange(record)
                                        "
                                    />
                                    <a-tooltip
                                        placement="top"
                                        :title="`Sales Price Changed from ${formatAmountCurrency(
                                            record.default_sales_price
                                        )} to ${formatAmountCurrency(
                                            record.sales_price
                                        )}.`"
                                        v-if="record.is_sales_price_changed"
                                    >
                                        <InfoCircleOutlined
                                            style="color: #faad14"
                                        />
...
export default {
    ...
    setup({ emit }) {
        ...
        const { purchaseItemColumns } = fields();
        const {
            state,
            orderType,
            orderPageObject,
        ...
        const sampleFileUrl = window.config.purchase_sample_file;
        ...
        const handleSalesPriceChange = (record) => {
            selectedProducts.value = selectedProducts.value.map((product) => {
                if (product.xid === record.xid) {
                    return {
                        ...product,
                        is_sales_price_changed:
                            record.sales_price < record.default_sales_price &&
