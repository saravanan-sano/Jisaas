<template>
    <a-button type="primary" @click="handleDownload">B2C Report</a-button>
</template>
<script>
    ...
    methods: {
        async handleDownload() {
            try {
                ...
                let products = response.data.filter((item) => {
                    ...
                    const userWithGstno = item?.user?.tax_number == null;

                    // Filter by payment_status equals "paid"
                    const isPaid = item.payment_status === "paid";

                    return isInSelectedRange && isPaid && userWithGstno;
                });
        ...
        exportToExcel(products) {
            const productsWithCustomColumns = products.map((product) => {
                ...
                return {
                    "Bill no": product.invoice_number,
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, "B2C");
        ...
        saveExcelFile(buffer) {
            ...
            const fileName = `B2C_${moment().format("DD_MM_yy")}.xlsx`;
