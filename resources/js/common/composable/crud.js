...
const crud = () => {
    ...
    const editItem = (item) => {
        ...
        Object.keys(initData.value).forEach((key) => {
            if (has(multiDimension, key)) {
                ...
                if (key == "wholesale" && itemDetails.wholesale.length == 0) {
                    itemDetails[key] = [
                        {
                            id: "",
                            start_quantity: 0,
    ...
    const showDeleteConfirm = (id) => {
        Modal.confirm({
            ...
            onOk() {
                return axiosAdmin
                    .delete(
                        `${crudUrl.value}/${id}?user_type=${formData.value?.user_type}`
                    )
    ...
    const restFormData = () => {
        // For Resting the wholesale[] For products
        if ("wholesale" in initData.value) {
            formData.value = {
                ...initData.value,
                wholesale: [
                    {
