                        ...
                        <div class="relative flex flex-col px-9 pt-8 pb-9 h-full bg-white bg-opacity-90 border-blueGray-100 rounded-4xl shadow-9xl backdrop-46">
                            ...
                            <div class="relative z-10">
                                ...
                                <h3 class="mb-1 text-4xl text-gray-900 font-bold leading-none">
                                    ...
                                    @if($subscriptionPlan->monthly_price == 0)
    <span class="monthly_div">CALL US</span>
@else
    <span class="monthly_div">
        {{ $subscriptionPlan->currency_symbol }}{{ $subscriptionPlan->monthly_price }}/{{ $frontSetting->pricing_month_text }}
    </span>
@endif

@if($subscriptionPlan->annual_price == 0)
    <span class="yearly_div" style="display: none;">CALL US</span>
@else
    <span class="yearly_div" style="display: none;">
        {{ $subscriptionPlan->currency_symbol }}{{ $subscriptionPlan->annual_price }}/{{ isset($frontSetting->pricing_year_text) ? $frontSetting->pricing_year_text : 'year' }}
    </span>
@endif

                                   
                                </h3>
                                @if ($subscriptionPlan->monthly_price != 0 || $subscriptionPlan->annual_price != 0) {
                                ...
                                <ul class="mb-10">
                                    @foreach ($subscriptionPlan->features as $subscriptionPlanFeature)
                                        <li class="mb-4 flex items-center font-heading font-semibold text-base">
                                            <img class="mr-2" src="{{ asset('front/images/check1.png') }}" alt="">
                                            <p>{{ $subscriptionPlanFeature }}</p>
                                        </li>
                                    @endforeach
                                </ul>
                                <a href="{{ route('main', ['path' => '/register']) }}" class="py-4 px-10 w-full text-white font-semibold rounded-xl focus:ring focus:ring-indigo-300 bg-indigo-600 hover:bg-indigo-700 transition ease-in-out duration-200" type="button">
