...
<section class="pt-14 pb-14 bg-gray-100 overflow-hidden">
    <div class="container mx-auto px-4">
        ...
        <p class="client-description">{{ $frontSetting->client_description }}</p>
        <div class="flex flex-wrap justify-center -m-10">
            @foreach ($frontClients as $frontClient)
                <div class="w-auto p-5">
                    <img src="{{ $frontClient->logo_url }}" alt="{{ $frontClient->name }}" class="client-img">
                    <h2 class="text-center">{{ $frontClient->name }}</h2>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="container mx-auto px-8">
        <div class="flex @if($loop->even) flex-wrap md:flex-row-reverse @else flex-wrap @endif -m-6">
          ...
          <div class="w-full lg:w-auto p-3">
              <a href="{{ $frontSetting->header_button1_url }}"  class="group relative font-heading font-semibold w-full md:w-auto py-5 px-8 text-xs text-white bg-purple-600 hover:bg-gray-800 uppercase overflow-hidden rounded-md tracking-px">
    ...
    <div class="container mx-auto px-8">
        ...
        <div class="flex flex-wrap justify-center items-center -m-1">
          ...
          <div class="w-full lg:w-auto p-3 text-center">
              <a href="{{ $frontSetting->header_button1_url }}"  class="group relative font-heading font-semibold w-full md:w-auto py-5 px-8 text-xs text-white bg-gray-900 hover:bg-gray-800 uppercase overflow-hidden rounded-md tracking-px">
                  {{ $frontSetting->header_button1_text }}
              </a>
          </div>
          </div>
