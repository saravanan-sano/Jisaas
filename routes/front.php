...
use App\Http\Controllers\Api\Front\CartItemController;
...
ApiRoute::group(['prefix' => 'front', 'namespace' => 'App\Http\Controllers\Api\Front'], function () {
    ...
    ApiRoute::get('homepage/{warehouse}/{productId}', ['as' => 'api.front.product_id', 'uses' => 'HomePageController@front_homepage_productsById']);
    ApiRoute::get('homepage_products/{warehouse}', ['as' => 'api.front.homepage', 'uses' => 'HomePageController@front_homepage_products']);
    ...
    ApiRoute::group(['middleware' => ['api.front.check']], function () {
        ...
        ApiRoute::group(['prefix' => 'self'], function () {
            ...
            // Define the GET route to fetch all cart items
            ApiRoute::post('cart-items', [CartItemController::class, 'store_cart']); // For storing cart items
            ApiRoute::get('cart-items/{userId}', [CartItemController::class, 'get_cart']); // For retrieving cart items
            ApiRoute::put('cart-items/{id}/update-status', [CartItemController::class, 'updateStatus']);
        });
    });
});
